...
.PHONY: test-handlers test-stores test-nats test-integration test-full setup help deps test credo dialyzer coverage check format clean release publish-release setup-hooks setup-db reset-db logs git-push push-and-publish sync-release-version pre-push-cleanup
...
help:
	...
	@echo "  make logs            - Tail fitness_bot log with grc (brew install grc; make -C .. install-grc)"
	@echo ""
...
compile:
	@LOG_FILE="/tmp/compile-fitness-$$(date +%s).log"; \
	echo "Compiling fitness and logging to $$LOG_FILE..."; \
...
release: check
	...
	@echo "==============================================="
	rm -rf _build/prod/rel/fitness_bot
	MIX_ENV=prod $(MIX) release
	@echo ""
	@echo "✓ Release built successfully"
	@echo "Location: _build/prod/rel/fitness_bot/"
	@echo ""
...
test-release-smoke:
	...
	@echo "==============================================="
	@RELEASE_NAME=fitness_bot NATS_SERVERS=nats://localhost:4224 \
...
logs:
	@$(SCRIPTS_DIRECTORY)/tailbot_log.sh
