    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/bot_army_fitness/bin/bot_army_fitness"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyFitness.Release.migrate()'"

          $RELEASE_BIN eval 'BotArmyFitness.Release.migrate()' || {
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_fitness/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_fitness/releases/start_erl.data)
        fi
