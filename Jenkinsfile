pipeline {
  ...
  environment {
    BOT_NAME = 'fitness_bot'
    STATE_NAME = 'fitness_bot'
    RELEASE_DIR = "/opt/ergon/releases/${BOT_NAME}"
    GITHUB_REPO = "ergon-automation-labs/ergon-fitness"
    ...
    stage('Publish Deploy Event') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/fitness_bot/bin/fitness_bot"
...
EOF
            ...
            /opt/bot_army/scripts/nats_publish.sh ops.deploy.fitness_bot "$PAYLOAD" || echo "⚠️  NATS publish failed (non-blocking)"
            exit 1
          fi
...
EOF
          ...
          $RELEASE_BIN eval 'FitnessBot.Release.migrate()' && {
            echo "✓ Migrations complete"
            echo "Publishing deploy_complete event..."
            VERSION=$(awk '{print $2}' "$RELEASE_DIR/current/fitness_bot/releases/start_erl.data" 2>/dev/null || echo "unknown")
            PAYLOAD=$(cat <<EOF
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          echo "Running: $RELEASE_BIN eval 'FitnessBot.Release.migrate()'"
